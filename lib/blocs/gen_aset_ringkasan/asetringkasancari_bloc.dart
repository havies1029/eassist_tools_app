...
import 'package:eassist_tools_app/repositories/gen_aset_ringkasan/asetringkasancari_repository.dart';
  ...
  Future<void> onFetchAsetRingkasanCari(FetchAsetRingkasanCariEvent event,
    ...
    if (state.status == ListStatus.initial) {
      List<AsetRingkasanCariModel> items =
          await repo.getAsetRingkasanCari(state.searchText, 0);
    ...
    List<AsetRingkasanCariModel> items =
        await repo.getAsetRingkasanCari(state.searchText, state.hal);
    if (items.isEmpty) {
