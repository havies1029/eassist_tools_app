...
class SimulcargoCrudAPI {
  Future<ReturnDataAPI> simulcargoCrudTambahAPI(
  ...
  Future<bool> simulcargoCrudHapusAPI(String simulcargoId) async {
    String hapusEndpoint =
        "${AppData.prefixEndPoint}/api/simulcargo/simulcargocrud/delete";
  ...
  Future<SimulcargoCrudModel> simulcargoCrudLihatAPI(
      String simulcargoId) async {
    String lihatEndpoint =
        "${AppData.prefixEndPoint}/api/simulcargo/simulcargocrud/read";
  ...
  Future<SimulcargoCrudModel> simulCargoCrudInitValueAPI() async {
    ...
    if (response.statusCode == 200) {
      debugPrint("simulCargoCrudInitValueAPI : ${jsonEncode(response.body)}");
  ...
  Future<ReturnDataAPI> simulCargoCrudCalcPremiAPI(
      SimulcargoCrudModel record) async {
    debugPrint("simulCargoCrudCalcPremiAPI");

    String tambahEndpoint =
        "${AppData.prefixEndPoint}/api/simulcargo/simulcargocrud/calcpremi";
    Map<String, String> queryParams = {
      "modul_id": "simulCargoCrudCalcPremiAPI"
    };
    var uri =
        AppData.uriHtpp(AppData.httpAuthority, tambahEndpoint, queryParams);
